...
from bela.joint_el_heads import (ClassificationMetrics, ClassificationHead, SaliencyClassificationHead, SpanEncoder)
                ...
                if length != 0:
                    if md_score >= self.md_threshold:
                        if (
                            ...
                            # flat_mentions_repr
                            # flat_entities_repr
                            novel_prediction_example.append([offset, length, float(el_scores[cand_idx]), float(cand_scores[cand_idx][0]), float(md_score), flat_mentions_repr, flat_entities_repr, int(cand_indices[
                            cand_idx])])
                    cand_idx += 1
