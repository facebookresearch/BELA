...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    for line in input:
        ...
        if tag == 'page':
            ...
            title = m.group(3)
        elif tag == 'timestamp' and not timestamp:
            timestamp = m.group(3)
        elif tag == 'timestamp' and timestamp:
            ...
            if (colon < 0 or (title[:colon] in acceptedNamespaces) and id != last_id and
                ...
                job = (id, revid, urlbase, title, timestamp, revtimestamp, page, ordinal)
...
def main():
    ...
    if args.article:
        ...
        with open(input_file) as file:
            ...
            timestamps = re.findall(r'<timestamp>(\d*?)</timestamp>', page)
            timestamp = timestamps[0] if ids else ''
            revtimestamps = timestamps[1] if len(ids) > 1 else ''
            if m:
                title = m.group(1)
            else:
            ...
            if m:
                ...
                urlbase = ''
            Extractor(id, revid, urlbase, title, timestamp, revtimestamp, [page]).extract(sys.stdout)
        return
